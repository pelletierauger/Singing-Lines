...
Ribbon.prototype.upgrade = function() {
    ...
    if (amplitude > 0.01) {
        this.lines[this.currentNumber].blurFactor = 0.5;
        this.lines[this.currentNumber].color.a = amplitude * 2;
    }
...
Ribbon.prototype.show = function() {
    for (let i = 0; i < this.lines.length; i += 1) {
        let x = this.lines[i].x + Math.random() * 0.1;
...
Line.prototype.upgrade = function() {
    if (this.blurFactor > 0.05) {
        this.blurFactor -= 0.0125;
    }
    if (this.color.a > 0.0) {
        this.color.a -= 0.0125;
    }
